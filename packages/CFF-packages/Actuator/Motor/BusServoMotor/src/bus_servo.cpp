...
bool ServoPort::readData(uint8_t id, uint8_t address, uint8_t length, uint8_t *data, uint8_t *error)
{
    ...
    if (rx_len > 0)
    {
        result = recv_unpack(recv_buffer_, id, rx_len, error, data);
    }
    else
    {
        is_using_ = false;
...
bool Servo::setAngle(float angle)
{
    ...
    {
        LOG_W("Goal angle out of range. now angle %.2f", angle);
        return false;
    }
...
bool Servo::setPwm(int16_t time)
{
    if(time<0)
    {
		time = -time;
