...
TM1638QYF::TM1638QYF(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, TM16xx_PIN_TYPE strobePin, bool activateDisplay, uint8_t intensity)
...
void TM1638QYF::setSegments(uint8_t segments, uint8_t position)
{ // set 8 leds on common grd as specified
  ...
  if (position < _maxDisplays)
  {
    ...
    *((uint64_t *)buf) = flipDiagA8H1(*((uint64_t *)buf));
    ...
    for (uint8_t nPos = 0; nPos < TM1638QYF_MAX_POS; nPos++)
    {
