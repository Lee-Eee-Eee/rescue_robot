...
TM1630::TM1630(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, TM16xx_PIN_TYPE strobePin, uint8_t numDigits, bool activateDisplay, uint8_t intensity)
...
void TM1630::setSegments(uint8_t segments, uint8_t position)
{ // set 8 leds on common grd as specified
  ...
  if (position < _maxDisplays)
    sendData(position << 1, (uint8_t)(segments << 1) & 0xFE);
}

void TM1630::setSegments16(uint16_t segments, uint8_t position)
{ // method to send more than 8 segments (9 max for TM1630)
  ...
  {
    sendData(position << 1, (uint8_t)(segments << 1) & 0xFE);
    sendData((position << 1) | 1, (uint8_t)(segments << 2) & 0x20);
  }
}
