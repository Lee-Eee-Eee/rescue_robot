...
TM1652::TM1652(TM16xx_PIN_TYPE dataPin, uint8_t numDigits, bool activateDisplay, uint8_t intensity, uint8_t displaymode)
...
void TM1652::begin(bool activateDisplay, uint8_t intensity)
{ // Call begin() in setup() to clear the display and set initial activation and intensity.
...
#define TM1652_BITDELAY 49 // NOTE: core 1.0.6 of LGT8F328@32MHz miscalculates delayMicroseconds() (should be 52us delay). For fix see https://github.com/dbuezas/lgt8fx/issues/18
  ...
  for (int nBit = 0; nBit < 8; nBit++)
  {
    if (data & 1)
      fParity = !fParity;
    TM16xx_PIN_WRITE(dataPin, (data & 1) ? PIN_HIGH : PIN_LOW);
    data >>= 1;
    TM16xx_DELAY(TM1652_BITDELAY);
  }

  // parity - low when odd
  TM16xx_PIN_WRITE(dataPin, fParity);
  TM16xx_DELAY(TM1652_BITDELAY);
...
#define TM1652_WAITCMD 3
  // static uint32_t tLastCmd=micros();
  while (TM16xx_GET_TIME() - tLastCmd < TM1652_WAITCMD)
    ; // wait 3 ms to ensure next byte is interpreted as a command
  // tLastCmd=micros();
  //  NOTE: CONSTRUCTORS SHOULD NOT CALL DELAY() <= gives hanging on certain ESP8266 cores as well as on LGT8F328P
  //  Using micros() or millis() in constructor also gave issues on LST8F328P
}
...
{ // signal the end of a command, to remember the timing
  tLastCmd = TM16xx_GET_TIME();
}

void TM1652::sendData(uint8_t address, uint8_t data)
...
void TM1652::sendCommand(uint8_t cmd)
...
void TM1652::setupDisplay(bool active, uint8_t intensity)
