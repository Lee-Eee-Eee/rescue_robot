...
TM1640Anode::TM1640Anode(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, uint8_t numDigits, bool activateDisplay, uint8_t intensity)
...
void TM1640Anode::sendAsciiChar(uint8_t pos, char c, bool fDot)
{ // Method to send an Ascii character to the display.
  ...
  uint16_t uSegments = *(TM16XX_FONT_15SEG + (c - 32));
  setSegments16(uSegments | (fDot ? 0b10000000 : 0), pos);
}

void TM1640Anode::setSegments(uint8_t segments, uint8_t position)
...
void TM1640Anode::setSegmentMap(const uint8_t aMap[])
...
{ // Map the segments to another location if that's requested.
  ...
  if (pSegmentMap)
  {
    ...
    */
    ...
    for (uint8_t n = 0; n < _maxSegments; n++)
    {
...
void TM1640Anode::setSegments16(uint16_t segments, uint8_t position)
{ // Set 16 leds on common grd as specified
  ...
  if (position < TM1640Anode_MAX_POS)
  {
    ...
    for (uint8_t nSeg = 0; nSeg < _maxSegments; nSeg++)
    {
      uint8_t nVal = 0;
      for (uint8_t nPos = 0; nPos < TM1640Anode_MAX_POS; nPos++)
      {
