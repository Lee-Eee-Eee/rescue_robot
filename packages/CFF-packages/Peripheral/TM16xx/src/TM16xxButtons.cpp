...
// constructor
TM16xxButtons::TM16xxButtons(TM16xx *pTM16xx, uint8_t nMaxButtons) : _pTM16xx(pTM16xx), _nMaxButtons(nMaxButtons)
...
uint8_t TM16xxButtons::findSlot(uint8_t nButton, uint8_t nStateFind)
{
  for (uint8_t n = 0; n < _nMaxButtons; n++)
  {
...
bool TM16xxButtons::isPressed(uint8_t nButton)
{
  // if(nButton>=_nMaxButtons) return(false);
  nButton = findSlot(nButton);
  if (nButton == TM16XX_BUTTONS_SLOT_NOTFOUND)
    return (false);
...
// function to get the current long pressed state
bool TM16xxButtons::isLongPressed(uint8_t nButton)
{
  nButton = findSlot(nButton);
  if (nButton == TM16XX_BUTTONS_SLOT_NOTFOUND)
    return (false);
...
int TM16xxButtons::getPressedTicks(uint8_t nButton)
{
  nButton = findSlot(nButton);
  if (nButton == TM16XX_BUTTONS_SLOT_NOTFOUND)
    return (0);
...
#else
  // do a tick for each possible button
  for (uint8_t n = 0; n < 32; n++) // uint32_t dwButtons has 32 buttons at max
    tick(n, dwButtons & bit(n));   // MMOLE 181103: _BV only works on 16-bit values!
#endif
  return (dwButtons);
}
 ...
 */
void TM16xxButtons::tick(uint8_t nButtonNum, bool activeLevel)
{
  uint16_t now = (uint16_t)TM16xx_GET_TIME(); // current (relative) time in msecs. To safe RAM we only use the bottom word (16 bits for instead of 32 for approx. 50 days)

  // find the slot of the button used, or prepare new slot
  uint8_t nSlot = findSlot(nButtonNum);
  if (nSlot == TM16XX_BUTTONS_SLOT_NOTFOUND)
  { // no slot for same button, find a new one
    if (!activeLevel)
      return; // new slots only get occupied when activated
