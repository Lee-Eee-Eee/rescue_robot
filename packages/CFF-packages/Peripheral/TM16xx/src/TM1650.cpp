...
TM1650::TM1650(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, uint8_t numDigits, bool activateDisplay, uint8_t intensity, uint8_t displaymode)
...
{ // send a byte to the chip the way the TM1650 likes it (MSB-first)
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    TM16xx_PIN_WRITE(dataPin, data & 0x80 ? PIN_HIGH : PIN_LOW); // in contrast to other TM16xx chips, the TM1650 expects MSB first
    data <<= 1;
...
{ // Keyscan data on the TM1650/TM1637 is one byte, with index of the button that is pressed.
  ...
  if (received <= 0x47)
    received = received - 0x44;
  else if (received <= 0x4F)
    received = received - 0x4C + 4;
  else if (received <= 0x57)
    received = received - 0x54 + 8;
  else if (received <= 0x5F)
    received = received - 0x5C + 12;
  else if (received <= 0x67)
    received = received - 0x64 + 16;
  else if (received <= 0x6F)
    received = received - 0x6C + 20;
  else if (received <= 0x77)
    received = received - 0x74 + 24;
