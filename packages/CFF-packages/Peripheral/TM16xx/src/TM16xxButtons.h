...
extern "C"
{
  typedef void (*callbackTM16xxButtons)(uint8_t nButton);
#if (TM16XX_OPT_BUTTONS_EVENT)
...
{
  uint8_t button;     // number of the button in the slot (0-31 or TM16XX_BUTTONS_SLOT_UNUSED)
  uint8_t state;      // current state of the button tracked, initially TM16XX_BUTTONS_STATE_START;
...
{
public:
  TM16xxButtons(TM16xx *pTM16xx, uint8_t nMaxButtons = TM16XX_BUTTONS_MAXBUTTONSLOTS);
   ...
   */
  void tick(uint8_t nButton, bool level);

  bool isPressed(uint8_t nButton);
  bool isLongPressed(uint8_t nButton);
  int getPressedTicks(uint8_t nButton);
  void reset(void);
...
private:
  uint8_t _nMaxButtons;                // maximum number of buttons tracked (can be less than TM16XX_BUTTONS_MAXBUTTONSLOTS when using malloc)
...
#endif
  uint8_t findSlot(uint8_t nButton, uint8_t nStateFind = TM16XX_BUTTONS_SLOT_ANYSTATE);
};
#endif
