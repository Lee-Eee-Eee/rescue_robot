...
namespace auto_ctrl
{
    ...
    // void AutoControl_SetSpeed(Robot_t *robot, float linear_x, float linear_y, float angular_z, float delay_s);
    // void AutoControl_SetPosition(Robot_t *robot, float x, float y, float yaw);
    // void AutoControl_VisualFindBall(Robot_t *robot, uint8_t color);
}
 ...
 *
 * @note 
 *    - 根据机器人的底盘类型（Omni, Three-wheel Omni, Two-wheel Differential, Four-wheel Differential）初始化底盘运动学。
...
void setup()
{
    ...
    /* 初始化Robot控制模式和底盘参数 */
    robot.ctrl_state = AUTO_OPERATION;   // 设置机器人控制状态为启动状态	AUTO_OPERATION 自动模式
    ...
    chassis->init(WHEELS_DIAMETER, OMNI4_WHEELS_TRACK, OMNI4_WHEELS_BASE, HUB_MOTOR_MAX_SPEED);
    ...
    chassis->init(WHEELS_DIAMETER, DIFF_WHEELS_TRACK, DIFF_WHEELS_BASE, HUB_MOTOR_MAX_SPEED);
#endif

    // 初始化PID控制器
    position_x_pid.Init(&robot.pid_pos_x.input, 
                &robot.pid_pos_x.output,
                &robot.pid_pos_x.setpoint,
                0.95, 0, 0, _PID_CD_DIRECT); // 初始化X轴位置PID控制器
    position_x_pid.SetSampleTime(1000/PID_CALC_FREQUENCY); // 设置采样时间
    position_x_pid.SetOutputLimits(-CHASSIS_MAX_LINEAR_SPEED, CHASSIS_MAX_LINEAR_SPEED); // 设置输出限制
    position_x_pid.SetMode(_PID_MODE_AUTOMATIC); // 设置PID模式为自动

    position_y_pid.Init(&robot.pid_pos_y.input, 
                &robot.pid_pos_y.output,
                &robot.pid_pos_y.setpoint,
                0.95, 0, 0, _PID_CD_DIRECT); // 初始化Y轴位置PID控制器
    position_y_pid.SetSampleTime(1000/PID_CALC_FREQUENCY); // 设置采样时间
    position_y_pid.SetOutputLimits(-CHASSIS_MAX_LINEAR_SPEED, CHASSIS_MAX_LINEAR_SPEED); // 设置输出限制
    position_y_pid.SetMode(_PID_MODE_AUTOMATIC); // 设置PID模式为自动

    yaw_pid.Init(&robot.pid_yaw.input, 
                &robot.pid_yaw.output,
                &robot.pid_yaw.setpoint,
                0.15, 0, 0, _PID_CD_DIRECT); // 初始化偏航角PID控制器
    yaw_pid.SetSampleTime(1000/PID_CALC_FREQUENCY); // 设置采样时间
    yaw_pid.SetOutputLimits(-CHASSIS_MAX_ANGULAR_SPEED, CHASSIS_MAX_ANGULAR_SPEED); // 设置输出限制
    yaw_pid.SetMode(_PID_MODE_AUTOMATIC); // 设置PID模式为自动

    /* 创建电机控制对象 */
    robot.motor[0] = new HubMotor(motor_port, LEFT_FRONT_MOTOR_ID); // 创建左前电机对象
    robot.motor[1] = new HubMotor(motor_port, RIGHT_FRONT_MOTOR_ID); // 创建右前电机对象
    robot.motor[2] = new HubMotor(motor_port, LEFT_BACK_MOTOR_ID); // 创建左后电机对象
    robot.motor[3] = new HubMotor(motor_port, RIGHT_BACK_MOTOR_ID); // 创建右后电机对象
    ...
    robot.servo[0] = new Servo(servo_port, 1, ServoType::SCS,135,-135); // 创建SCS舵机对象
    robot.servo[1] = new Servo(servo_port, 2, ServoType::SCS,135,-135); // 创建SCS舵机对象
		robot.servo[2] = new Servo(servo_port, 3, ServoType::SCS,135,-135); // 创建SCS舵机对象
    robot.servo[0]->setAngle(-60); // 设置舵机0的角度为-60度（夹爪张开或框抬起）
    robot.servo[0]->setAcceleration(150); // 设置舵机0的加速度为150
		robot.servo[0]->setAngle(-60); // 设置舵机0的角度为-60度（夹爪张开或框抬起）
    robot.servo[1]->setAngle(124); // 设置舵机1的角度为124度（夹爪张开或框抬起）
    robot.servo[1]->setAcceleration(150); // 设置舵机1的加速度为150
		robot.servo[1]->setAngle(124);// 设置舵机1的角度为124度（夹爪张开或框抬起）
		robot.servo[2]->setAngle(108); // 设置舵机2的角度为108度（云台pitch）
    robot.servo[2]->setAcceleration(75); // 设置舵机2的加速度为75
		robot.servo[2]->setAngle(108);// 设置舵机2的角度为108度（云台pitch）
...
void loop()
{
    ...
    // 切换机器人控制状态
    robot_ctrl_state(&robot, IS_BUTTON_CLICKED(gamepad->buttons, robot.last_buttons, GAMEPAD_XBOX_BUTTON_HOME));

    // 判断是否为手动控制状态
    if (robot.ctrl_state == MANUAL_OPERATION)
    {
        // 处理手动控制子状态的切换，通过按下特定按钮
        manual_ctrl::SubCtrlState(&robot, IS_BUTTON_CLICKED(gamepad->buttons, robot.last_buttons, GAMEPAD_XBOX_BUTTON_MINUS));
 ...
 * @note 
 *    - 该函数在自动控制线程中被调用。
 *    - `auto_ctrl::Finish()` 函数在自动阶段完成后被调用，用于执行必要的清理操作。
 */
void auto_mode_path(void *parameter)
{
    while (1)
...
void motor_reset()
	{if (robot.ctrl_state != DEBUG_STATE)
    {
		for (int i = 0; i < 4; ++i) {
			...
			delay(1);
     if (robot.motor[i]->readEncoder()->system_fault_code==2){
				robot.motor[i]->clearStatus();
		 }
    }
	}
