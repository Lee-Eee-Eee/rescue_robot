...
void SetSpeed(float linear_x, float linear_y, float angular_z, float delay_s);  //跑速度的函数（x上的速度，y上速度，z上旋转速度，时间） 建议每次只给一个方向上的速度和时间
void MovePosition(float x, float y, float yaw); //跑里程计点位（x位置，y位置，yaw轴位置）
...
bool certain_home(Robot_t *robot, uint8_t color , uint8_t pd); //判断安全区是否在画面中心
int certain_ball(Robot_t *robot, uint8_t color , uint8_t pd); //判断是否有球函数 #使用此函数需把云台架高至合适高度
...
void WaitStart()
{
    ...
    while (1)
    {
        ...
        if (robot_->ctrl_state == AUTO_OPERATION && robot_->auto_sub_state == START)//
        {
        ...
        case 3: 
            ...
            break;
        case 4://开环自动路径 
					/*此处是给你写固定路线放球的地方  #上场前必须准备的保底方案 可参考find1-4移动来写*/
						break;
        }
...
void SetSpeed(float linear_x, float linear_y, float angular_z, float delay_s)
{
		...
		// 延迟时间 (delay_s) 乘以 1000 转换为毫秒，再除以每次循环的延迟时间 (_SET_SPEED_TIMEOUT)
 ...
 */
bool IsWithinTolerance(struct position *target, struct position *current, float pos_tol=0.03, float yaw_tol=1){
		#if defined(ROBOT_CHASSIS_TWO_DIFF) // 两轮差速
		if(robot.target_pos.yaw == 90 or robot.target_pos.yaw == -90 or // 判断目标yaw角度是否为 +-90或+-270
			robot.target_pos.yaw == 270 or robot.target_pos.yaw == -270)
		{
			if(fabs(robot.target_pos.y-robot.current_pos.y) < pos_tol && // 判断目标y和现在的y是否在差值内
        fabs(robot.target_pos.yaw-robot.current_pos.yaw) < 1)// 判断目标yaw和现在的yaw是否在差值内
			{
				return true;// 返回 true
			}
		}
		if(robot.target_pos.yaw == 180 or robot.target_pos.yaw == 0 or robot.target_pos.yaw == -180)// 判断目标yaw角度是否为 0 或 +-180
		{
			if(fabs(robot.target_pos.x-robot.current_pos.x) < pos_tol &&// 判断目标x和现在的x是否在差值内
        fabs(robot.target_pos.yaw-robot.current_pos.yaw) < 1)// 判断目标yaw和现在的yaw是否在差值内
			{
				return true;// 返回 true
			}
		...
		#endif
    if (fabs(target->x - current->x) < pos_tol &&// 判断目标x和现在的x是否在差值内
        fabs(target->y - current->y) < pos_tol &&// 判断目标y和现在的y是否在差值内
        fabs(target->yaw - current->yaw) < yaw_tol)// 判断目标yaw和现在的yaw是否在差值内
    {
        return true;// 返回 true
    }
    return false;// 返回 false
...
void MovePosition(float x, float y, float yaw)
{
    ...
    while (!IsWithinTolerance(&robot_->target_pos, &robot_->current_pos))// 判断是否到位
    {
		if(robot.ctrl_state == MANUAL_OPERATION || robot.ctrl_state == STARTUP_STATE){
			break;// 切换模式退出
			}
        delay(10);// 延时10ms
    }
...
void Turn_angle() {
    ...
    float angleInDegrees = RADIAN_TO_DEGREE(angleInRadians);
		...
		robot_->target_pos.yaw = LIMIT_ANGLE_180(angleInDegrees);// 目标YAW为转向本方安全区方向
		while (robot_->ctrl_state == AUTO_OPERATION){// 自动模式下
		if (fabs(robot.target_pos.yaw-robot.current_pos.yaw) < 1){// 判断目标角度和现在角度差值绝对值是否小于1度
		break;// 退出循环
		}
		delay(10);// 延时10ms
		}
...
void Turn_angle2() {
    ...
    float angleInDegrees = RADIAN_TO_DEGREE(angleInRadians);
		...
		robot_->target_pos.x = robot_->current_pos.x;// 目标X为现在X
		robot_->target_pos.y = robot_->current_pos.y;// 目标Y为现在Y
		robot_->target_pos.yaw = LIMIT_ANGLE_180(angleInDegrees);// 目标YAW为转向（x2，y2）方向
		while (robot_->ctrl_state == AUTO_OPERATION){// 自动模式下
		if (fabs(robot.target_pos.yaw-robot.current_pos.yaw) < 1){// 判断目标角度和现在角度差值绝对值是否小于1度
		break;// 退出循环
		}
...
void corner_angle(){
	bool pd1;
	if (fabs((robot_->current_pos.yaw)+90)<75){// 判断目前角度是否满足90度摆放
			...
			{
			if (fabs(robot.target_pos.yaw-robot.current_pos.yaw) < 1)//目标陀螺仪角度与现在陀螺仪角度插值是否小于 1
			{
			break;// 退出循环
			}
			delay(10);// 延时10ms
			...
			{
			MovePosition(robot_->current_pos.x,robot_->current_pos.y,0);//小车姿态保持0度
			SetSpeed(-0.3, 0, 0, 0.75);// 向后倒退一段时间
			MovePosition(robot_->current_pos.x,robot_->current_pos.y,-90);// 小车姿态保持-90度
			SetSpeed(0.3, 0, 0, 1.25);// 向前移动一段时间
			}
			...
			SetSpeed(0.3, 0, 0, 1.25);// 向前移动一段时间
	 ...
	 else if (fabs((robot_->current_pos.yaw))<60){//判断车的姿态绝对值是为-60-60度
					robot.imu_hold = true;//陀螺仪保持
					robot.chassis_mode = POSTION_MODE;// 小车进入里程计模式
					robot_->target_pos.x = robot_->current_pos.x;// x目标位置为现在位置
					robot_->target_pos.y = robot_->current_pos.y;// y目标位置为现在位置
				...
				else if (fabs(fabs(robot_->current_pos.yaw)-180)<60){//判断车的姿态绝对值是否为120-240度
					robot.imu_hold = true;//陀螺仪保持
					robot.chassis_mode = POSTION_MODE;// 小车进入里程计模式
					robot_->target_pos.x = robot_->current_pos.x;// x目标位置为现在位置
					robot_->target_pos.y = robot_->current_pos.y;// y目标位置为现在位置
...
void go_home(){
    ...
    robot_->set_vel.angular_z = 0;// z速度给0
		while (robot_->ctrl_state == AUTO_OPERATION){ //自动模式下
		//计算目标位置和实际位置插值
		float distance = sqrt(pow(robot_->current_pos.x - (home_xx+(home_x-1.05)), 2) + pow(robot_->current_pos.y - (home_yy+(home_y+0.95)), 2));
		if (distance<0.25){// 判断目标位置和实际位置差值是否小于0.25m
		break; // 退出循环
		}
		delay(10);// 延时10ms
		}
    ...
    robot_->set_vel.angular_z = 0;// z速度给0
		while (robot_->ctrl_state == AUTO_OPERATION){ //自动模式下
		//计算目标位置和实际位置插值
		float distance = sqrt(pow(robot_->current_pos.x - home_x, 2) + pow(robot_->current_pos.y - home_y, 2));
...
{		
    ...
    while (1)
    {
          ...
          ball_tmp = visual::getBall(i);// 获取视觉下发数据
            ...
            if (color == ball_tmp->type)// 判断寻找颜色是否相符
            {
                ...
                if(dt < 100)// 判断是否小与100ms（最新消息）
                {
									...
									xx = 160-(ball_tmp->x + ball_tmp->w/2);//320*320  理想情况是160装歪可能需要略调
									}
									yy = 160-(ball_tmp->y + ball_tmp->h/2);//320*320  理想情况是160装歪可能需要略调
        ...
        if (find_ball != NULL)// 判断是否获取到符合条件的数据集合
        {
            ...
            x = 160-(find_ball->x + find_ball->w/2);//320*320  理想情况是160装歪可能需要略调
						}
            y = 160-(find_ball->y + find_ball->h/2);//320*320  理想情况是160装歪可能需要略调
            robot->set_vel.linear_x = y * 0.0065;// 跟随太慢改比例系数 反运动加正负号
            robot->set_vel.linear_y = 0;// 不适用横向平移量
            robot->set_vel.angular_z = x * 0.0075;// 跟随太慢改比例系数 反运动加正负号

            if (abs(x) <= 5 && abs(y)<4)// 判断X,Y的向量差是否满足区间
            {
								robot->imu_hold = true;// 陀螺仪保持
								robot->target_pos.yaw = LIMIT_ANGLE_180(robot->current_pos.yaw);// 保持目前角度
                return true;// 返回 true
            }
        }
				else{
...
void PART1(){
	...
	while(robot_->ctrl_state == AUTO_OPERATION){
		while (robot_->ctrl_state == AUTO_OPERATION){
				...
				if (mine<2){
					pd = VisualFindBall(&robot, a,0);// 0,0只找红球 1,0只找蓝色安全区 2,0只找红色安全区 3,0只找蓝色球 4,0只找黄球 5,0只找黑球 0,1除了红球以外的球 3,1除了蓝球以外的球
				}
				else{
				pd = VisualFindBall(&robot, b,1);// 0,0只找红球 1,0只找蓝色安全区 2,0只找红色安全区 3,0只找蓝色球 4,0只找黄球 5,0只找黑球 0,1除了红球以外的球 3,1除了蓝球以外的球
				}
				if (pd){
				robot.imu_hold = true;//陀螺仪保持
		...
		while (robot_->ctrl_state == AUTO_OPERATION){
		pd = VisualFindBall(&robot, c,0);// 0,0只找红球 1,0只找蓝色安全区 2,0只找红色安全区 3,0只找蓝色球 4,0只找黄球 5,0只找黑球 0,1除了红球以外的球 3,1除了蓝球以外的球
		if(pd){
				robot.imu_hold = true;//陀螺仪保持
				delay(300);// 延时300ms	
				corner_angle();//调整车身姿态
...
void find2() {
    ...
    MovePosition(robot_->current_pos.x, robot_->current_pos.y, 0);
    // 移动机器人到计算后的家位置，仍然面朝0度
    MovePosition(0.4 + (home_x - 1.05), (home_y + 0.95), 0); // 到达目标位置(1.05, 0, 0)
    delay(100); // 等待100毫秒，以确保机器人到达目标位置
    // 将机器人转动到90度面朝
    MovePosition(robot_->current_pos.x, robot_->current_pos.y, 90);
    // 移动机器人到新的计算位置，面朝90度
    MovePosition(0.4 + (home_x - 1.05), -0.5 + (home_y + 0.95), 90); // 到达目标位置(1.05, 0, 90)
    delay(300); // 等待300毫秒，以确保机器人到达目标位置
}
...
void find3() {
    ...
    MovePosition(robot_->current_pos.x, robot_->current_pos.y, 0);
    // 移动机器人到计算后的家位置，仍然面朝0度
    MovePosition(1.8 + (home_x - 1.05), (home_y + 0.95), 0); // 到达目标位置(1.05, 0, 0)
    delay(100); // 等待100毫秒，以确保机器人到达目标位置
    // 将机器人转动到90度面朝
    MovePosition(robot_->current_pos.x, robot_->current_pos.y, 90);
    // 移动机器人到新的计算位置，面朝90度
    MovePosition(1.8 + (home_x - 1.05), -0.5 + (home_y + 0.95), 90); // 到达目标位置(1.05, 0, 90)
    delay(300); // 等待300毫秒，以确保机器人到达目标位置
}
