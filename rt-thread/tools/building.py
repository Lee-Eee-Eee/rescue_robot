...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    if os.getenv('KCONFIG_CONFIG'):
        kconfig_path = os.path.normpath(os.getenv('KCONFIG_CONFIG') + '/../')
        f = open(os.path.join(kconfig_path, 'rtconfig.h'), 'r')
    else:
        f = open('rtconfig.h', 'r')
...
def GenTargetProject(program = None):
    ...
    if not os.path.exists(project_folder):
        ...
        print("Folder created:", project_folder)
    # else:
    #     print("Folder already exists:", project_folder)

    if GetOption('target') in ['mdk', 'mdk4', 'mdk5']:
        from keil import MDK2Project, MDK4Project, MDK5Project, ARMCC_Version

        mdk_project_folder =  os.path.join(project_folder, 'MDK')
        if not os.path.exists(mdk_project_folder):
            os.makedirs(mdk_project_folder)

            source = os.path.join(os.getenv('BSP_ROOT'), "board", "linker_scripts")
            destination = os.path.join(mdk_project_folder, "board", "linker_scripts")
        ...
        if os.path.isfile(os.getenv('BSP_ROOT')+'/template.uvprojx') and GetOption('target') not in ['mdk4']: # Keil5
            MDK5Project(mdk_project_folder+'/'+GetOption('project-name') + '.uvprojx', Projects)
            print("Keil5 project is generating...")
        elif os.path.isfile(os.getenv('BSP_ROOT')+'/template.uvproj') and GetOption('target') not in ['mdk5']: # Keil4
            MDK4Project(mdk_project_folder+'/'+GetOption('project-name') + '.uvproj', Projects)
            print("Keil4 project is generating...")
        elif os.path.isfile(os.getenv('BSP_ROOT')+'/template.Uv2') and GetOption('target') not in ['mdk4', 'mdk5']: # Keil2
            MDK2Project(mdk_project_folder+'/'+GetOption('project-name') + '.Uv2', Projects)
