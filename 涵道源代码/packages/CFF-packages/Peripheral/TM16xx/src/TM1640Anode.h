...
public:
	...
	TM1640Anode(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, uint8_t numDigits = 4, bool activateDisplay = true, uint8_t intensity = 7);

	/** Set an Ascii character on a specific location (overloaded for 15-segment display) */
	virtual void sendAsciiChar(uint8_t pos, char c, bool dot); // public method to allow calling from TM16xxDisplay

	/** Set the segments at a specific position on or off */
	virtual void setSegments(uint8_t segments, uint8_t position); // will duplicate G to G1/G2 in 15-segment
	...
	virtual void setSegmentMap(const uint8_t aMap[]);
...
private:
	...
	const uint8_t *pSegmentMap = NULL;	  // pointer to segment map set using setSegmentMap()
};
