...
TM1668::TM1668(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, TM16xx_PIN_TYPE strobePin, uint8_t numDigits, bool activateDisplay, uint8_t intensity, uint8_t displaymode)
...
void TM1668::setSegments(uint8_t segments, uint8_t position)
...
void TM1668::setSegments16(uint16_t segments, uint8_t position)
{ // method to send more than 8 segments (13 max for TM1668)
  ...
  if (position < _maxDisplays)
  {
    sendData(position << 1, (uint8_t)segments & 0xFF);
    sendData((position << 1) | 1, (uint8_t)(segments >> 8));
  }
}

void TM1668::setLED(uint8_t color, uint8_t pos)
{
...
{ // Common anode RGB LEDs can be connected on the 10 segments and GRID5-7
  ...
  uint16_t wSegmentsR = 0;
  uint16_t wSegmentsG = 0;
  uint16_t wSegmentsB = 0;
...
void TM1668::setRGBLED(uint8_t color, uint8_t pos)
