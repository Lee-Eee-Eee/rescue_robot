...
public:
  ...
  TM1638Anode(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, TM16xx_PIN_TYPE strobePin, bool activateDisplay = true, uint8_t intensity = 7);
...
private:
  uint8_t bitmap[TM1638Anode_MAX_POS]; // store a bitmap for all 8 digit to allow common anode manipulation
};
