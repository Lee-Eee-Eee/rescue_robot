...
TM1638Anode::TM1638Anode(TM16xx_PIN_TYPE dataPin, TM16xx_PIN_TYPE clockPin, TM16xx_PIN_TYPE strobePin, bool activateDisplay, uint8_t intensity)
...
void TM1638Anode::setSegments(uint8_t segments, uint8_t position)
{	// set 10 leds on common grd as specified
	...
	if (position < _maxDisplays)
	{
		...
		for (uint8_t nPos = 0; nPos < _maxSegments; nPos++)
		{
			uint16_t btVal = 0;
			for (uint8_t j = 0; j < TM1638Anode_MAX_POS; j++)
			{
			...
			sendData(nPos << 1, (uint8_t)btVal & 0xFF);
			// send 2nd byte of data
			sendData((nPos << 1) | 1, (uint8_t)(btVal >> 8) & 0x03);
		}
	}
}
...
{
	for (uint8_t nPos = 0; nPos < TM1638Anode_MAX_POS; nPos++)
	{
		// all OFF
		sendData(nPos << 1, 0);
		sendData((nPos << 1) | 1, 0);
