...
void calculateInverseKinematicsFor3WheelOmni(float wheels_rpm[], float linear_x, float linear_y, float angular_z, ChassisParam *param)
{
    ...
    speed_temp[0] =-sqrt(3)/2.f*x_rpm + 0.5*y_rpm + tan_rpm;    // left_front 
    speed_temp[1] = sqrt(3)/2.f*x_rpm + 0.5*y_rpm + tan_rpm;    // right_front
...
void calculateForwardKinematicsFor3WheelOmni(ChassisVelocities *vel, float lf_spd, float rf_spd, float lb_spd, ChassisParam *param)
{
}
