...
def MDK4Project(target, script):
    if os.path.isfile(os.getenv('BSP_ROOT')+'/template.uvproj') is False:
        print ('Warning: The template project file [template.uvproj] not found!')
        return

    template_tree = etree.parse(os.getenv('BSP_ROOT')+'/template.uvproj')
    ...
    # copy uvopt file
    if os.path.exists(os.getenv('BSP_ROOT')+'/template.uvopt'):
        import shutil
        shutil.copy2(os.getenv('BSP_ROOT')+'/template.uvopt', '{}.uvopt'.format(os.path.splitext(target)[0]))

def MDK5Project(target, script):
    if os.path.isfile(os.getenv('BSP_ROOT')+'/template.uvprojx') is False:
        print ('Warning: The template project file [template.uvprojx] not found!')
        return

    template_tree = etree.parse(os.getenv('BSP_ROOT')+'/template.uvprojx')
    ...
    if os.path.isfile(project_uvopt):
        os.unlink(project_uvopt)
    # copy uvopt file
    if os.path.exists(os.getenv('BSP_ROOT')+'/template.uvoptx'):
        import shutil
        shutil.copy2(os.getenv('BSP_ROOT')+'/template.uvoptx', '{}.uvoptx'.format(os.path.splitext(target)[0]))

def MDK2Project(target, script):
    template = open(os.getenv('BSP_ROOT')+'template.Uv2', "r")
